define(['lib/Module'], function (Module) {
    ...
    function fireEvent(moduleName, event, data, cb) {
        data = data || {};
        callHooks(moduleName ? 'on_' + moduleName + '_' + event : 'on_' + event, cb, data);
        if (event !== 'eventFired') {
    ...
    function doAction(action, data, cb) {
        data = data || {};
        callHooks('do_' + action, cb, data);
    }

    function callHooks(hook, cb) {
