define(function () {
    ...
    function callHooks(prefix, op) {
        ...
        modules.forEach(function (module) {
            ...
            if (hookFunc && module.isEnabled()) {
                modulesWithHook.push(module);
                checkFilters(module, op, function () {
                    setTimeout(function () {
        ...
        if(modulesWithHook.length === 0) {
            console && console.log && console.log('ModuleManager: no modules contain hook - '+hook);
        }

        function checkFilters(module, op, cb) {
            window.async.every(modules, function (mod, next) {
                if (mod.filterEvents) {
                    var type = /^on/.test(prefix) ? 'event' : 'action';
                    mod.filterEvents({module:module, event:op, args:args, type:type}, function (val) {
                        next(val !== false);
                    })
                } else {
                    next(true);
